...
static gboolean gdl_dock_paned_draw(GtkWidget *widget, cairo_t *cr);
                     ...
                     GValue           *other_data)
{
	...
	gboolean  child1resize = FALSE;/* hacked */
	gboolean  child2resize = FALSE;/* hacked */
	gboolean  child1shrink = FALSE;/* hacked */
	gboolean  child2shrink = FALSE;/* hacked */
	...
	g_object_get_property(requestor, "resize", &value);
	child1resize = g_value_get_boolean(&value);
	g_object_get_property(requestor, "resize", &value);
	child2resize = g_value_get_boolean(&value);
	
	g_object_get_property(requestor, "shrink", &value);
	child1shrink = g_value_get_boolean(&value);
	g_object_get_property(requestor, "shrink", &value);
	child2shrink = g_value_get_boolean(&value);
	/*g_print("resize = %d", g_value_get_boolean(&value));*/
	/* hacked */
    ...
    switch (gdl_dock_item_get_orientation (GDL_DOCK_ITEM (object))) {
        case GTK_ORIENTATION_HORIZONTAL:
            if (!child1 && position == GDL_DOCK_LEFT) {
				gtk_paned_pack1 (paned, GTK_WIDGET (requestor), child1resize, child1shrink);/* hacked */
                done = TRUE;
            } else if (!child2 && position == GDL_DOCK_RIGHT) {
				gtk_paned_pack2 (paned, GTK_WIDGET (requestor), child2resize, child2shrink);/* hacked */
                done = TRUE;
            }
            break;
...
static gboolean gdl_dock_paned_draw(GtkWidget *widget, cairo_t *cr)
{
	...
	pos = gdl_dock_item_get_orientation (widget);
	
	cairo_set_source_rgb(cr, 0.99, 0.99, 0.99);
	cairo_rectangle(cr, allocation.x, allocation.y, allocation.width, allocation.height);
	cairo_fill(cr);
	
	paned = gdl_dock_item_get_child (item);
	if (paned && GTK_IS_PANED (paned))
	{
		gint position = gtk_paned_get_position (GTK_PANED (paned));
		cairo_set_source_rgb(cr, 0.83, 0.83, 0.83);
		if(pos==GTK_ORIENTATION_HORIZONTAL)
		{
			cairo_rectangle(cr, position+1, 0, 1, allocation.height);
		}
		else
		{
			cairo_rectangle(cr, allocation.x, position+1, allocation.width, 1);
		}
		cairo_fill(cr);
	}
	
	GTK_WIDGET_CLASS(gdl_dock_paned_parent_class)->draw(widget, cr);
	return FALSE;
}
